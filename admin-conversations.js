/* ═══════════════════════════════════════════════════════════════════════
   ...
   Carga todas las conversaciones de todas las empresas, las muestra
   en 3 columnas: lista · chat · ficha comercial del lead.
...
(function () {
  ...
  const inboxCompanyFilter = document.getElementById("inboxCompanyFilter");
  ...
  if (!inboxList) return; // not on this page

  /* ── State ────────────────────────────────────────────────────────── */
  let allConversations = [];   // { ...conv, companyName }
  let allLeads = [];           // raw leads from /api/leads
  let allCompanies = [];       // [ { companyId, name } ]
  let activeConv = null;       // currently selected conversation object
  ...
  function channelIcon(ch) {
    const icons = {
      ...
      messenger: `<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.373 0 0 4.974 0 11.111c0 3.498 1.744 6.614 4.469 8.654V24l4.088-2.242c1.092.3 2.246.464 3.443.464 6.627 0 12-4.974 12-11.111C24 4.974 18.627 0 12 0zm1.191 14.963l-3.055-3.26-5.963 3.26L10.732 8l3.131 3.259L19.752 8l-6.561 6.963z"/></svg>`,
      webchat:   `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21l4-4 4 4"/></svg>`,
    };
    return icons[ch] || `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/></svg>`;
  }
  ...
  function findLead(conv) {
    if (!conv) return null;
    // exact match by leadId = conversationId
    let lead = allLeads.find((l) => l.leadId === conv.conversationId && l.companyId === conv.companyId);
    if (lead) return lead;
    // match by phone in customerId
    if (conv.customerId) {
      const phone = conv.customerId.replace(/\D/g, "");
      lead = allLeads.find((l) => l.companyId === conv.companyId && l.phone && l.phone.replace(/\D/g, "").endsWith(phone.slice(-9)));
    }
    return lead || null;
  }
  ...
  async function loadAll() {
    try {
      inboxList.innerHTML = `<div class="inbox-loading">Cargando…</div>`;
      ...
      // /api/companies returns a plain array; /api/leads returns { leads: [] }
      allCompanies = Array.isArray(companiesRes) ? companiesRes : (companiesRes.companies || []);
      allLeads = Array.isArray(leadsRes) ? leadsRes : (leadsRes.leads || []);

      // Populate company filter
      inboxCompanyFilter.innerHTML = `<option value="all">Todas las empresas</option>` +
        allCompanies.map((c) => `<option value="${esc(c.companyId)}">${esc(c.name || c.companyId)}</option>`).join("");

      // Load conversations for all companies in parallel
      const convResults = await Promise.all(
        allCompanies.map((c) =>
          getJSON(`/api/conversations/${encodeURIComponent(c.companyId)}`)
            .then((r) => (r.conversations || []).map((cv) => ({
              ...cv,
              companyName: c.name || c.companyId,
            })))
            .catch(() => [])
        )
      );

      allConversations = convResults.flat().sort((a, b) =>
  ...
  function renderList() {
    ...
    if (company !== "all") items = items.filter((c) => c.companyId === company);

    if (query) {
      items = items.filter((c) => {
        const lead = findLead(c);
        const haystack = [
          c.customerName, c.summary, c.unit, c.companyName,
          lead?.email, lead?.phone, lead?.business,
    ...
    inboxList.innerHTML = items.map((conv) => {
      ...
      const isActive = activeConv && activeConv.conversationId === conv.conversationId && activeConv.companyId === conv.companyId;
  ...
  async function openConversation(companyId, conversationId) {
    ...
    inboxChatHeader.innerHTML = `
      <div class="inbox-chat-info">
        ...
        <div>
          ...
          <small>${esc(meta.companyName)} · ${esc(channelLabel(meta.channel))}</small>
        </div>
    ...
    try {
      const full = await getJSON(`/api/messages/${encodeURIComponent(companyId)}/${encodeURIComponent(conversationId)}`);
      renderMessages(full);
  ...
  function renderProfile(conv, lead) {
      ...
      <div class="inbox-profile-inner">
        ...
        <section class="inbox-profile-section">
          ...
          <dl class="inbox-profile-dl">
            ...
            <div><dt>Canal origen</dt><dd>${esc(channelLabel(conv.channel))}</dd></div>
            <div><dt>Empresa cliente</dt><dd>${esc(conv.companyName || "—")}</dd></div>
          </dl>
        </section>
        ...
        <section class="inbox-profile-section">
          ...
          <dl class="inbox-profile-dl">
            ...
            <div><dt>Valor estimado</dt><dd>${lead?.estimatedValue ? `$${Number(lead.estimatedValue).toLocaleString("es-CO")}` : "—"}</dd></div>
        ...
        <section class="inbox-profile-section">
          ...
          <dl class="inbox-profile-dl">
            ...
            <div><dt>Última actividad</dt><dd>${esc(fmtDateTime(conv.lastMessageAt || lead?.updatedAt))}</dd></div>
            <div><dt>Mensajes</dt><dd>${esc(String(conv.messageCount ?? (conv.messages?.length ?? "—")))}</dd></div>
            <div><dt>Estado conv.</dt><dd><span class="inbox-status-badge status-${esc(conv.status)}">${esc(statusLabel(conv.status))}</span></dd></div>
          </dl>
        </section>

        ${lead?.notes ? `
        <section class="inbox-profile-section">
          <h4 class="inbox-profile-section-title">Notas comerciales</h4>
          <p class="inbox-profile-notes">${esc(lead.notes)}</p>
        </section>` : ""}

        ${lead?.interest || conv.summary ? `
        <section class="inbox-profile-section">
          <h4 class="inbox-profile-section-title">Resumen</h4>
          <p class="inbox-profile-notes">${esc(lead?.interest || conv.summary)}</p>
        </section>` : ""}
    ...
    // "Ver en CRM" button → switch to clientes tab and open lead editor
    inboxProfile.querySelectorAll("[data-action='goto-crm']").forEach((btn) => {
      btn.addEventListener("click", () => {
        const crmLink = document.querySelector(".side-nav a[data-tab='clientes']");
        if (crmLink) crmLink.click();
        // platform.js will handle opening the dialog via openAdminLeadEditor
        // We trigger it after a short delay to let the tab render
        setTimeout(() => {
          const manageBtn = document.querySelector(
            `.manage-admin-lead[data-company-id="${btn.dataset.companyId}"][data-lead-id="${btn.dataset.leadId}"]`
          );
  ...
  inboxReplyForm?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const companyId = inboxReplyForm.dataset.companyId;
    const convId    = inboxReplyForm.dataset.convId;
    ...
    try {
      ...
      if (idx !== -1) {
        ...
        allConversations[idx].messageCount = (data.conversation.messages || []).length;
      }
      renderList();
    } catch (err) {
  ...
  /* ── Company filter ───────────────────────────────────────────────── */
  inboxCompanyFilter?.addEventListener("change", () => {
    activeCompany = inboxCompanyFilter.value;
    renderList();
  });
  ...
  document.querySelectorAll(".side-nav a[data-tab]").forEach((link) => {
    link.addEventListener("click", () => {
      if (link.dataset.tab === "conversaciones" && allConversations.length === 0) {
        loadAll();
      }
    });
  });

  // Also load if the page loads already on this tab (e.g. deep link)
